...
la stratégie avec une comparaison sur le taille des fichiers, il suffit de remplacer le pointeur de fonction
\verb|int (*ouichefs_fblocks_strategy)(struct inode *a, struct inode *b)| par la nouvelle fonction
qui permet de comparer deux inode.
...
\begin{itemize}
	\item Libération lorsque tous les inode d'un répertoire ont été utilisés.
	\item La suppression doit se faire si le i\_count est supérieur à 1 car on a remarqué que si le
	fichier est caché dans les d\_entry sont i\_count augumente de 1 alors qu'il n'est pas nécessairement
	utilisée, par exemple dans le cas de la creation d'un fichier.
	\item Changement de politique de suppression de fichiers.
	\item Déclanchement du mécanisme de libération depuis le userspace.
\end{itemize}

\section{Fonctionnalités à travailler}
	Lorsqu'il ne reste que x\% de blocs libres les blocs sont bien libérés mais les
	fichiers ont l'air corrempus ceci est peut-être du à une mauvaise fonction de suppression
	 qui ne libere pas bien les méta-données car la somme des fichiers
	 crée dépasse la taille de la partition mais la libération des blocs à partir du pourcentage
	 est bien fonctionnelle car le nombre de blocs libre auguemante de la taille du fichier supprimé
	 dès que l'on atteint le seuil fixé.
